...
export const StatCard = styled.div<{ color: string; size?: 'sm' | 'md' | 'lg' }>`
  display: flex;
  align-items: stretch;
  ...
  overflow: hidden;
  min-height: ${(props) => (props.size === 'sm' ? '30px' : props.size === 'md' ? '36px' : '44px')};
`;

export const StatCardValue = styled.div<{ color: string; size?: 'sm' | 'md' | 'lg' }>`
  display: flex;
  align-items: center;
  justify-content: center;
  padding: ${(props) =>
    props.size === 'sm' ? '4px 10px' : props.size === 'md' ? '5px 10px' : '6px 12px'};
  font-size: ${(props) =>
    props.size === 'sm' ? '1.4rem' : props.size === 'md' ? '1.6rem' : '2rem'};
  font-weight: 700;
...
export const StatCardDivider = styled.div<{ color: string; size?: 'sm' | 'md' | 'lg' }>`
  width: 1px;
  height: ${(props) => (props.size === 'sm' ? '18px' : props.size === 'md' ? '22px' : '26px')};
  align-self: center;
  background: ${(props) => props.color}35;
  flex-shrink: 0;
`;

export const StatCardLabel = styled.div<{ size?: 'sm' | 'md' | 'lg' }>`
  display: flex;
  align-items: center;
  padding: ${(props) =>
    props.size === 'sm' ? '4px 8px' : props.size === 'md' ? '5px 8px' : '6px 10px'};
  font-size: ${(props) =>
    props.size === 'sm' ? '1rem' : props.size === 'md' ? '1.05rem' : '1.1rem'};
  font-weight: 700;
...
export const FilterBadge = styled.div<{ color: string; active: boolean; disabled?: boolean }>`
  ...
  opacity: ${(props) => (props.disabled || !props.active ? 0.6 : 1)};
